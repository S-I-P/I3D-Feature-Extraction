...
from flow import flow_instances
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_threads', default=8, type=int,
                            help='number of threads for optical flow calculation')
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for root,_,files in os.walk(rootdir):
            for file in files:
                ...
                print("---------------------")
                print(file)
                instances = flow_instances(os.path.join(root, file), float(duration[file]), config, num_threads)
                ...
                np.save(os.path.join(saveAt,'flow.npy'), features)
                outfile.write(file)
                outfile.write("\n")
                outfile.flush()
